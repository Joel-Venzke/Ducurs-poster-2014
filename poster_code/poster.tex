...
\item {\bf\color{violet}The energy splitting is proportional to the electric field strength.}
\item For linearly polarized light, we can ``see'' only the two $m=0$ levels.
\item These levels form the {\bf\color{newgreen}Autler-Townes doublet} in the energy spectrum
...
\item Also, {\bf\color{violet} we vary the splitting by ramping on/off the pulse}. 
\end{itemize} 
...
\vspace{1cm}\begin{center}\pbox{0.8\columnwidth}{}{linewidth=2mm,framearc=0.1,linecolor=lightblue,fillstyle=gradient,
  ...
  \vspace{-2cm}
  \mycaption{????These plots illustrate that the triplet effect depends on the intensity, not on the energy of the laser pulse. Note, this effect is only present for pulses ending with the $\sin^2$ envelope. This suggests the shape of the field at the end of the pulse is significant.}
